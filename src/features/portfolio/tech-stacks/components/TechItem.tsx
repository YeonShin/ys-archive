  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div
          className={cn(
            'flex h-16 w-16 items-center justify-center rounded-2xl shadow-sm transition-all duration-300 md:h-20 md:w-20',
            isFilteredOut
        ...
        >
          {IconComponent ? (
            <IconComponent className="text-brand-neutral-light text-3xl md:text-4xl" />
          ) : (
            <span className="text-brand-neutral-light font-mono text-xl font-bold md:text-3xl">
      ...
      >
        <p className="text-brand-neutral-light font-bold">{tech.name}</p>
        {tech.level && (
          <p className="text-brand-neutral-light text-[10px] opacity-80">{tech.level}</p>
        )}
      </TooltipContent>
    </Tooltip>
  );
